...
```{r gsea, eval=performing_gsea}

gene_list <- read.csv(tsv_allGenes, sep="\t", row.names=1) %>%
        mutate(gene_name = c(rownames(res))) %>%
        relocate(gene_name, .before=1)
...
tsv_gsea <- sprintf("%s/gsea_list_%s.tsv", outdir, "test_gsea")
write.table(fgseaResTidy, tsv_gsea, sep="\t", row.names = FALSE)
