...
# QUESTION: Do we even need these?
...
#de_common_df <- data.frame(Contrast=sapply(contrast, toString), Gene=sapply(common_genes, toString))
...
hist(efit$p.value[,1], breaks=0:40/40, main=sprintf("%s \n - efit",compared_A), xlim=c(0,1), xlab="p-value")
hist(efit$p.value[,2], breaks=0:40/40, main=sprintf("%s \n - efit",compared_B), xlim=c(0,1), xlab="p-value")
...
hist(tfit$p.value[,1], breaks=0:40/40, main=sprintf("%s \n - tfit",compared_A), xlim=c(0,1), xlab="p-value")
hist(tfit$p.value[,2], breaks=0:40/40, main=sprintf("%s \n - tfit",compared_B), xlim=c(0,1), xlab="p-value")
...
# GSEA 
```{r gsea}
performing_gsea(tsv_allGenes_A, compared_A)
performing_gsea(tsv_allGenes_B, compared_B)

#gene sets
idx <- ids2indices(msigdbr_list,id=rownames(v)) 
cam_A <- camera(v,idx,design,contrast=contr.matrix[,1])
head(cam_A,5)
cam_B <- camera(v,idx,design,contrast=contr.matrix[,2])
head(cam_B,5)

# ??????
barcodeplot(efit$t[,1], index=idx[[rownames(cam_A[1,])]], index2=idx[[rownames(cam_A[2,])]], main=compared_A)
barcodeplot(efit$t[,1], index=idx[[rownames(cam_B[1,])]], index2=idx[[rownames(cam_B[2,])]], main=compared_B)
```
...
# heatmap
a_topgenes <- res_A$gene_name[1:100] #first 100 top-ranked genes from a linear model fit
i_a <- which(v$genes$gene_name %in% a_topgenes)
mycol <- colorpanel(1000,"blue","white","red")
heatmap.2(lcpm[i_a,], scale="row",
   labCol=rownames(coldata), labRow = NULL,
   col=mycol, trace="none", density.info="none", dendrogram="column", main=compared_A)

b_topgenes <- res_B$gene_name[1:100]
...
pheatmap(lcpm[i_b,],cluster_rows=FALSE, show_rownames=TRUE, fontsize_row=5, cluster_cols=TRUE, annotation_col=df, annotation_colors = ann_colors, main=sprintf("heatmap"))
