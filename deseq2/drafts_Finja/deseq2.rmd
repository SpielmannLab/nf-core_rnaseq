...
calculate_DEG <- function(res = res, name = name){
  if(list_DEG){
    ...
    write.table(DEGs_res, tsv_DEG, sep="\t")
  }
}
...
listing_genes <- function(res = res, name = name){
  if(list_all_genes){
    ...
    res_df <- res_df %>%
        mutate(gene_name = c(rownames(res_df))) %>%
        relocate(gene_name, .before=1)
    tsv_allGenes <- sprintf("%s/geneList_%s.tsv", outdir, name)
    write.table(res_df, tsv_allGenes, sep="\t")
  }
  return(tsv_allGenes)
}
...
[List of de genes](`r tsv_allGenes`)
...
if(any(perform_batch_correction == "RUV")){
  pcaPlot_ruv <- perform_pca(coldata_ruv, x = "W1", y = "W2", title="RUV")
  print(pcaPlot_ruv)
}

if(any(perform_batch_correction == "SVA")){
  pcaPlot_sva <- perform_pca(coldata_sva, x = "SV1", y = "SV2", title="SVA")
  print(pcaPlot_sva)
}
...
hist(res$pvalue, breaks=0:40/40, main=sprintf("%s \n - non-adjusted pvalue",compared), xlim=c(0,1), cex.main=.65, xlab="p-value")
hist(res$padj, breaks=0:40/40, main=sprintf("%s \n - adjusted pvalue",compared), xlim=c(0,1), cex.main=.65, xlab="adjusted p-value")
```
...
if(any(perform_batch_correction == "RUV")){
  cat("### RUV Histograms \n")
  par(mfrow = c(1, 2))
  hist(resruv$pvalue, breaks=0:40/40, main=sprintf("%s \n - non-adjusted pvalue - RUV",compared), cex.main=.65, xlab="p-value")
  hist(resruv$padj, breaks=0:40/40, main=sprintf("%s \n - adjusted pvalue - RUV",compared), cex.main=.65, xlab="adjusted p-value")
}
```
...
if(any(perform_batch_correction == "SVA")){
  cat("### SVA Histograms \n")
  par(mfrow = c(1, 2))
  hist(ressva$pvalue, breaks=0:40/40, main=sprintf("%s \n - non-adjusted pvalue - SVA",compared), cex.main=.65, xlab="p-value")
  hist(ressva$padj, breaks=0:40/40, main=sprintf("%s \n - adjusted pvalue - SVA",compared), cex.main=.65, xlab="adjusted p-value")
}
```
...
pheatmap(assay(obj)[topVarGenes,],cluster_rows=FALSE, show_rownames=FALSE, cluster_cols=TRUE, annotation_col=df, annotation_colors = ann_colors, main=sprintf("heatmap - %s",used))
